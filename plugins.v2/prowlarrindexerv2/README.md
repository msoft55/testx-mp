...
集成 Prowlarr 的所有已配置索引器到 MoviePilot，实现统一搜索管理。

## ✨ 功能特性

### 核心功能

- ✅ **自动同步索引器** - 自动从 Prowlarr 同步已启用的索引器
...
- **英文关键词优化**: 自动过滤非英文搜索关键词（Prowlarr 对中文支持有限）

### 分类支持

> [!NOTE]
> 插件会自动从 Prowlarr 获取每个索引器支持的分类，并按 MoviePilot 标准格式转换。
...
2. 点击右上角齿轮图标，添加仓库地址：
   ...
   ```
3. 点击更新按钮，在插件列表中找到 **Prowlarr索引器** 并安装
...
### 第一步：获取 Prowlarr API 密钥

1. 登录 Prowlarr Web 界面
2. 进入 **设置 (Settings) → 通用 (General) → 安全 (Security)**
3. 找到 **API密钥 (API Key)** 字段
4. 复制 API 密钥
...
| **API密钥** | 在 Prowlarr 设置中获取的 API 密钥 | `1234567890abcdef...` | ✅ |
...
> [!TIP]
> **服务器地址示例**:
> - 本地部署：`http://localhost:9696`
> - 局域网：`http://192.168.1.100:9696`
> - Docker 内网：`http://prowlarr:9696`
> - 公网：`https://prowlarr.example.com`
...
1. 打开 **设置 → 插件 → Prowlarr索引器**，查看插件详情页
2. 在索引器列表中，找到每个站点的 **domain**（如 `prowlarr_indexer.2`）
3. 打开 **设置 → 站点管理 → 添加站点**
...
- 当 MoviePilot 需要通过代理才能访问 Prowlarr 时启用
- 使用 MoviePilot 系统设置中配置的代理服务器
- 如果 Prowlarr 在本地网络，通常不需要代理
...
> [!NOTE]
> 这是已知限制。插件使用虚拟域名（如 `prowlarr_indexer.2`）注册站点，MoviePilot 的站点测试会尝试 DNS 解析这些域名，因此可能失败。
...
**解决方案**:
- **忽略该错误** - 不影响搜索功能
- 站点连通性通过 Prowlarr 本身的测试功能验证
- 在 Prowlarr 管理界面中查看索引器状态
...
**A**: Prowlarr 对中文关键词支持有限，插件会自动过滤非英文关键词。
...
| `未获取到索引器列表` | 在 Prowlarr 中配置并启用索引器 |
...
【Prowlarr索引器】开始检索站点：Prowlarr索引器-M-Team - TP，关键词：The Matrix
【Prowlarr索引器】搜索完成：Prowlarr索引器-M-Team - TP 从 125 条原始结果中解析出 120 个有效结果
```
...
**Prowlarr 日志**:
1. **设置 → 通用 → 日志**，选择 **Debug**
2. **设置 → 系统 → 日志 → 文件**，下载最新日志

---

## 📖 API 文档

Prowlarr 和 Jackett 使用 Torznab 协议进行通信。详细的 API 文档请查看：

**[Prowlarr/Jackett API 文档](API%20Documents.md)**
...
**提交时请：**
- 选择插件：**Prowlarr索引器**
- 提供版本信息（MoviePilot、Prowlarr、插件）
- 描述详细的复现步骤
- 粘贴完整的日志（MoviePilot DEBUG + Prowlarr Debug）
- 附上配置截图（隐藏敏感信息）
...
[返回主页](../../README.md) • [查看 API 文档](API%20Documents.md)
